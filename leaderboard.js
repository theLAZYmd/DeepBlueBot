...
function Leaderboard(server, config) {
	...
	this.server = server;
	this.page = config  && config.page ? config.page : 1; 
    	this.type = config && config.type ? config.type : null;
   	this.active = config && config.active ? config.active : false;
}
	...
	for(serverID in this.data) {
		...
		for(userID in users) {
			...
			if(this.type === "Classical") {
				...
				if(ratingData.rapid === ratingData.maxRating) {
					ratingObj.type = "Rapid";
...
Leaderboard.prototype.getRank = function(getNick, userID) {
	...
	let	userData = this.data[serverID][userID]; 
	if(!userData) {
